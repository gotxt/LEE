...
| PHASE 2 | 150 | 0.5초 | 1.05초 | 0.1초 | 0.45초 | 저장 순서 순환 | 꺼짐 | 켜짐 |
...
## 6. 타임라인 밖에 남아 있는 수정 공격

2페이즈의 `legacyCrystals=true`가 활성화한다. `TraceStrikeGame.CrystalPatternLoop/FireCrystalBlast`와 `CrystalRules`를 사용하며 위 170개 이벤트에는 포함되지 않는다.

- 의도된 수정 수는 4개. 초기 위치는 논리 중심의 상하좌우 가장자리 안쪽을 찾는다. 실제 수는 맵과 연결성 보호에 따라 줄 수 있다.
- 수정은 통과할 수 없는 장애물이다. START/END 후보와 전장 연결을 보존하도록 배치를 조정한다.
- 수정 주변 radius=2의 체커 영역, 수정 중심 제외. 0.7초 경고 후 단발 폭발, 0.18초 피격 유예.
- 기본 공격 주기는 5초, 초기 타이머에는 인덱스×0.22초의 시차가 더해진다.
- 해당 페이즈 체력이 절반 이하(현재 75 이하)가 되었고 살아 있으면 위치를 한 번 재배치한다. 재배치 후 인덱스 0/1은 5초, 2/3은 4초 주기를 사용한다.
- 기존 불꽃/흙먼지/사운드 코루틴 연출을 사용한다. 새 UI 프리팹을 만든 것과 별개로 이 코드를 프리팹 호출로 전부 교체한 상태는 아니다.
- 별도 코루틴 타이머이므로 주 타임라인과 동일하게 일시 정지된다고 가정하지 않는다. 일반 전투에서 주 패턴과의 겹침을 확인한다.
