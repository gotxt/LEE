...
├─ Resources/Effects/
│  ├─ Prefabs/Impact/
│  │  ├─ VFX_TileImpact.prefab
│  │  ├─ VFX_CrystalSparks.prefab
│  │  ├─ VFX_DirtLaneEruption.prefab
│  │  └─ VFX_DirtAreaExplosion.prefab
│  └─ Audio/
│     ├─ Warning.wav
│     └─ Impact.wav
...
|---|---|---|
| VFX_TileImpact | 기존 Impact VFX의 주황색 타일 섬광 | 0.3초 이상 |
| VFX_CrystalSparks | 수정 공격에서 사용하던 불꽃 파편 4개 | 0.12초 이상 |
| VFX_DirtLaneEruption | 흙먼지 4개와 튀어 오르는 흙·돌 파편 12개 | 0.4초 이상 |
| VFX_DirtAreaExplosion | 충격파·먼지 6개·방사형 파편 40개 | 0.5초 이상 |
...
기존 패턴에 자동으로 프리팹을 덮어쓰지는 않았다. 새로 생성하는 기본 골렘 패턴은 `VFX_TileImpact`를 기본 VFX로 참조한다.

## 주의 사항 / 수정

- 대상 **타일마다 한 개씩** 생성된다. 흙 폭발을 넓은 영역 전체에 쓰면 파편이 매우 많아진다. 큰 폭발은 적은 수의 타일에 사용하고, 넓은 영역에는 VFX_TileImpact나 VFX_CrystalSparks를 권장한다.
