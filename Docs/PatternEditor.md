...
보스 인카운터의 **공유 위치 그룹**은 `snapshotKey`와 다르다. 위치 그룹은 패턴 실행 시작 시 기준 타일만 한 번 선택한다(맵 중앙, 시작 시 플레이어, 기믹/호출 위치, 고정 좌표, 랜덤 이동 가능 타일). 여러 경고·타격 쌍을 같은 그룹에 연결하면 각 공격의 `cells`와 `offset`을 기준 타일에 대해 따로 적용하므로, 서로 다른 타일을 서로 다른 시간에 공격할 수 있다. `snapshotKey`는 한 공격의 경고·타격이 최종 타일 집합을 공유하는 용도로 그대로 사용한다. 랜덤 그룹은 후보 타일을 맵에서 칠해 제한할 수 있으며, 제한하지 않으면 시작 시 이동 가능한 바닥 전체에서 뽑는다. 미리보기는 위치를 다시 뽑기 전까지 같은 결과를 유지한다. 독립 Shared Pattern Sequence 에셋에는 보스별 위치 그룹을 설정하지 않는다.
...
이동 봉인만 걸린 상태에서는 보스 공격 시간이 계속 흐른다. 수정 봉인 기믹도 주 타임라인과 같은 scaled time/inputLocked 규칙을 사용한다.
